...
function getShapeRadius(shapeIndex: number): number {
  ...
  const shapes = [
    <div className={`flex flex-col ${gap}`} key="z">
      <div className={`flex ${gap}`}>
        <div className={size} style={style} />
        <div className={size} style={style} />
      </div>
    ...
    <div className={size} style={style} key="dot" />,
    ...
    <div className={`flex ${gap}`} key="line">
      {Array.from({ length: 4 }).map((_, i) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: static decorative shape, never reordered
        <div key={i} className={size} style={style} />
      ))}
    </div>,
...
export function AnimatedBackground() {
  ...
  return (
    <div className="fixed inset-0 z-[-1] overflow-hidden bg-background">
      <div
        className="pointer-events-none absolute -top-[24%] left-1/2 h-[40%] w-[78%] -translate-x-1/2 rounded-full"
        style={{
          background:
      ...
      {lineBlocks.map((lineBlock) => (
        <div
          key={lineBlock.id}
          className="pointer-events-none absolute flex gap-[1.5px]"
          style={{
            top: lineBlock.top,
            left: lineBlock.left,
